weather() {
	if [[ "$1" == "--help" || "$1" == "-h" ]]; then
		echo ""
		echo "${COLOR_HEADER}  ◆  weather — Terminal weather report${COLOR_RESET}"
		echo "${COLOR_BORDER}  ─────────────────────────────────────${COLOR_RESET}"
		echo ""
		echo "${COLOR_TEXT}  USAGE${COLOR_RESET}"
		echo "${COLOR_NORMAL}    weather [city]${COLOR_RESET}"
		echo ""
		echo "${COLOR_TEXT}  ARGUMENTS${COLOR_RESET}"
		echo "${COLOR_CURSOR}    city${COLOR_RESET}    ${COLOR_NORMAL}City name to fetch weather for (default: Khulna)${COLOR_RESET}"
		echo ""
		echo "${COLOR_TEXT}  EXAMPLES${COLOR_RESET}"
		echo "${COLOR_NORMAL}    weather               → current city (Khulna)${COLOR_RESET}"
		echo "${COLOR_NORMAL}    weather Dhaka         → weather for Dhaka${COLOR_RESET}"
		echo "${COLOR_NORMAL}    weather \"New York\"    → use quotes for multi-word cities${COLOR_RESET}"
		echo "${COLOR_NORMAL}    weather London        → weather for London${COLOR_RESET}"
		echo ""
		echo "${COLOR_TEXT}  OUTPUT INCLUDES${COLOR_RESET}"
		echo "${COLOR_NORMAL}    • Current temperature, feels like, condition${COLOR_RESET}"
		echo "${COLOR_NORMAL}    • Temperature bar (color-coded: blue → green → orange → red)${COLOR_RESET}"
		echo "${COLOR_NORMAL}    • Humidity, wind speed/direction, pressure, visibility, UV index${COLOR_RESET}"
		echo "${COLOR_NORMAL}    • 3-day forecast (today, tomorrow, day after)${COLOR_RESET}"
		echo ""
		echo "${COLOR_TEXT}  DEPENDENCIES${COLOR_RESET}"
		echo "${COLOR_NORMAL}    Required   curl${COLOR_RESET}"
		echo "${COLOR_NORMAL}    Optional   jq  (richer output — sudo apt install jq)${COLOR_RESET}"
		echo ""
		echo "${COLOR_TEXT}  NOTES${COLOR_RESET}"
		echo "${COLOR_NORMAL}    • Falls back to simple wttr.in output if jq is not installed${COLOR_RESET}"
		echo "${COLOR_NORMAL}    • Data source: wttr.in${COLOR_RESET}"
		echo ""
		return 0
	fi
	...
	if ! command -v jq &>/dev/null; then
		echo "${COLOR_WARNING}  ⚠  Installing 'jq' recommended for better output${COLOR_RESET}"
		echo "${COLOR_NORMAL}  Install with: ${COLOR_CURSOR}sudo apt install jq${COLOR_RESET} or ${COLOR_CURSOR}sudo pacman -S jq${COLOR_RESET}"
		echo

		echo "${COLOR_HEADER}  ╔═══════════════════════════════════════════════════╗${COLOR_RESET}"
		echo "${COLOR_HEADER}  ║${COLOR_RESET}                🌤️  ${COLOR_TEXT}WEATHER REPORT${COLOR_RESET}                 ${COLOR_HEADER}║${COLOR_RESET}"
		echo "${COLOR_HEADER}  ╚═══════════════════════════════════════════════════╝${COLOR_RESET}"
		echo
		echo "  ${COLOR_CURSOR}📍 Location:${COLOR_RESET} ${COLOR_TEXT}$city${COLOR_RESET}"
		echo
	...
	echo "${COLOR_HEADER}  ◆  Fetching weather data...${COLOR_RESET}"

	local weather_data
	weather_data=$(curl -s "wttr.in/$city?format=j1" 2>/dev/null)

	if [ -z "$weather_data" ]; then
		echo "${COLOR_ERROR}  ✗${COLOR_RESET} Failed to fetch weather data. Check your internet connection."
		return 1
	fi

	if echo "$weather_data" | grep -q "Unknown location"; then
		echo "${COLOR_ERROR}  ✗${COLOR_RESET} Unknown location: ${COLOR_TEXT}'$city'${COLOR_RESET}"
		echo "${COLOR_NORMAL}  Try: weather \"New York\" or weather Dhaka${COLOR_RESET}"
		return 1
	fi
	...
	if [ "$temp" = "N/A" ] || [ -z "$temp" ]; then
		echo "${COLOR_ERROR}  ✗${COLOR_RESET} Failed to parse weather data"
		return 1
	fi
	...
	*[Ff]og* | *[Mm]ist* | *[Hh]aze*) weather_icon="🌫️" ;;
	...
	echo
	echo "${COLOR_HEADER}  ╔═══════════════════════════════════════════════════╗${COLOR_RESET}"
	echo "${COLOR_HEADER}  ║${COLOR_RESET}                ${weather_icon}  ${COLOR_TEXT}WEATHER REPORT${COLOR_RESET}                 ${COLOR_HEADER}║${COLOR_RESET}"
	echo "${COLOR_HEADER}  ╚═══════════════════════════════════════════════════╝${COLOR_RESET}"
	echo

	echo "  ${COLOR_BORDER}┌─────────────────────────────────────────────────┐${COLOR_RESET}"
	echo "  ${COLOR_BORDER}│${COLOR_RESET}  ${COLOR_CURSOR}📍 Location${COLOR_RESET}                                   ${COLOR_BORDER}│${COLOR_RESET}"
	printf "  ${COLOR_BORDER}│${COLOR_RESET}     ${COLOR_TEXT}%-40s${COLOR_RESET}  ${COLOR_BORDER}│${COLOR_RESET}\n" "$city"
	echo "  ${COLOR_BORDER}└─────────────────────────────────────────────────┘${COLOR_RESET}"
	echo

	echo "  ${COLOR_BORDER}┌─────────────────────────────────────────────────┐${COLOR_RESET}"
	echo "  ${COLOR_BORDER}│${COLOR_RESET}  ${COLOR_CURSOR}🌡️  Current Conditions${COLOR_RESET}                       ${COLOR_BORDER}│${COLOR_RESET}"
	echo "  ${COLOR_BORDER}│${COLOR_RESET}                                                 ${COLOR_BORDER}│${COLOR_RESET}"
	...
	echo "  ${COLOR_BORDER}│${COLOR_RESET}  ${COLOR_CURSOR}0°${COLOR_RESET}     ${COLOR_SUCCESS}15°${COLOR_RESET}     ${COLOR_WARNING}30°${COLOR_RESET}     ${COLOR_ERROR}45°${COLOR_RESET}                   ${COLOR_BORDER}│${COLOR_RESET}"
	echo "  ${COLOR_BORDER}└─────────────────────────────────────────────────┘${COLOR_RESET}"
	echo

	echo "  ${COLOR_BORDER}┌─────────────────────────────────────────────────┐${COLOR_RESET}"
	echo "  ${COLOR_BORDER}│${COLOR_RESET}  ${COLOR_CURSOR}📊 Details${COLOR_RESET}                                    ${COLOR_BORDER}│${COLOR_RESET}"
	echo "  ${COLOR_BORDER}│${COLOR_RESET}                                                 ${COLOR_BORDER}│${COLOR_RESET}"
	...
	echo "${COLOR_HEADER}  ╔═══════════════════════════════════════════════════╗${COLOR_RESET}"
	echo "${COLOR_HEADER}  ║${COLOR_RESET}                  ${COLOR_TEXT}3-DAY FORECAST${COLOR_RESET}                  ${COLOR_HEADER}║${COLOR_RESET}"
	echo "${COLOR_HEADER}  ╚═══════════════════════════════════════════════════╝${COLOR_RESET}"
	echo
	...
	echo "${COLOR_BORDER}  ─────────────────────────────────────────────────${COLOR_RESET}"
	echo "  ${COLOR_NORMAL}Updated: $current_time${COLOR_RESET}"
	echo "  ${COLOR_NORMAL}Source: wttr.in${COLOR_RESET}"
	echo
}
